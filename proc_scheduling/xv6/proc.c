...
//my code===========================================================>>>>>>>>>
//update for each proc
void tickCounter(void){
    struct proc* p;
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        //check for state and update time
...
//           (wait time, run time, time wait for i/o, pid of child)
    ...
    for(;;){
        ...
        for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
            if(p->parent != proc)
                continue;
            havekids = 1;
            if(p->state == ZOMBIE){ //if zombie children is found
                *wtime = p->retime;
                *rtime = p->rutime;
                *iotime = p->stime;
                pid = p->pid;
        ...
        // if no kids found, release and return -1
        if(!havekids || proc->killed){
            release(&ptable.lock);
            *status = -1;
            return -1;
